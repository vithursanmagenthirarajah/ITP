...
const IssueTreatmentDetails = props => (
    <tr>
      ...
      <td>
        <Button variant="dark"> <Link to={"/updatetreat/"+props.IssueTreatmentDetails._id}>update</Link></Button> | <Button  variant="danger" href="#" onClick={() => { props.deleteIssueTreatmentDetails(props.IssueTreatmentDetails._id) }}>delete  </Button>
      </td>
    </tr>
  )
...
export default class ViewIssueTreatment extends Component {
    ...
    constructor(props) {
        ...
        this.deleteIssueTreatmentDetails= this.deleteIssueTreatmentDetails.bind(this)
    
        this.state = {IssueTreatment: []};
      }
    
      componentDidMount() {
        axios.get('http://localhost:5000/api/IssueTreatment/')
          .then(response => {
      ...
      deleteIssueTreatmentDetails(id) {
        axios.delete('http://localhost:5000/api/IssueTreatment/'+id)
      ...
      IssueTreatmentList() {
        return this.state.IssueTreatment.map(currentIssueTreatmentDetails => {
          return <IssueTreatmentDetails IssueTreatmentDetails={currentIssueTreatmentDetails} deleteIssueTreatmentDetails={this.deleteIssueTreatmentDetails} key={currentIssueTreatmentDetails._id}/>;
        })
      }
