...
const Drug = props => (
  <tr>
    ...
    <td>
    <Button variant="dark"> <Link to={"/edit/"+props.drug._id}>edit</Link> </Button> |  <Button variant="danger" href="#" onClick={() => { props.deleteDrug(props.drug._id) }}>delete</Button>
  </td>
...
export default class ViewDrug extends Component {
  ...
  componentDidMount() {
    axios.get('http://localhost:5000/api/drug/')
      .then(response => {
  ...
  deleteDrug(drugId) {
    axios.delete('http://localhost:5000/api/drug/'+drugId)
  ...
  DrugList() {
    return this.state.drug.map(currentDrug => {
      return <Drug drug={currentDrug} deleteDrug={this.deleteDrug} key={currentDrug.drugId}/>;
    })
